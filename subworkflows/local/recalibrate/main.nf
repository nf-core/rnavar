...
include { GATK4_APPLYBQSR as APPLYBQSR } from '../../../modules/nf-core/gatk4/applybqsr'
...
workflow RECALIBRATE {
    ...
    SAMTOOLS_INDEX(APPLYBQSR.out.bam)

    def bam_indices = SAMTOOLS_INDEX.out.bai
        .mix(SAMTOOLS_INDEX.out.csi)
        .mix(SAMTOOLS_INDEX.out.crai)

    def bam_recalibrated_index = APPLYBQSR.out.bam.join(bam_indices, failOnMismatch: true, failOnDuplicate: true)
